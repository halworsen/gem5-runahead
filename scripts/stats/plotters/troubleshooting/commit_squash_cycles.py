...
class CommitSquashCycles(Plotter):
    name = 'Commit squash cycles for various IFTs'
    fname = 'squash_cycles_ift_sensitivity'
    description = 'Squash cycles for commit for all different IFTs'
    ...
    def plot(self) -> None:
        ...
        # Get the squash cycles
        overflows = {b: [] for b in benchmarks}
        for bench, data in self.data.items():
            LOG.info(f'getting squash cycles for {bench}:')
            for run in runs:
                overflow = data[run]['system']['processor']['cores1']['core']['commit']['squashCycles']['values'][0]
